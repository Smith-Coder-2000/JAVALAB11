...
class MultithreadingDoc extends Thread {
    login obj;
    MultithreadingDoc(login obj){
        this.obj=obj;
    }
    public boolean run(int id,StringBuffer password)
    {
        try {
            // Displaying the thread that is running
            return(obj.validatedoc(id,password));
        }
        catch (Exception e) {
...
class MultithreadingPat extends Thread {
    login obj;
    MultithreadingPat(login obj){
        this.obj=obj;
    }
    public boolean run(int id,StringBuffer password)
    {
        try {
            return(obj.validatepat(id,password));
        }
        catch (Exception e) {
            System.out.println("Exception is caught");
            return false;
...
class MultithreadingStaff extends Thread {
    login obj;
    MultithreadingStaff(login obj){
        this.obj=obj;
    }
    public boolean run(int id,StringBuffer password)
    {
        try {
            return(obj.validatestaff(id,password));
        }
        catch (Exception e) {
            System.out.println("Exception is caught");
            return false;
...
class login extends doctor{
    ...
    public static void main(String args[]){
        ...
        MultithreadingDoc object1= new MultithreadingDoc(obj);
        MultithreadingPat object2= new MultithreadingPat(obj);
        MultithreadingStaff object3= new MultithreadingStaff(obj);
            ...
            switch (ch) {
                ...
                case 3:
                    if(login.doctor==false && login.staff==false && login.patient==false){
                        ...
                        pswd.append(passw);
                        doct=object1.run(ids,pswd);
                        object1.start();
                        staff=object3.run(ids,pswd);
                        object3.start();
                    }
                ...
                case 4:
                    ...
                    psw.append(pass);
                    doct=object1.run(id,psw);
                    object1.start();
                    pat=object2.run(id,psw);
                    object1.start();
                    staff=object3.run(id,psw);
                    object3.start();
                    }
